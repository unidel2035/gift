...
#### 3.5.1 Pilot Result: The Signal Has a Scale

On September 5, 2026 we completed an independent replication attempt (issue #789): our own open-weights implementation of the J-lens technique — layer-wise sensitivity of the top logit to residual streams, C(t) = Var_ℓ[s_ℓ(t)] / mean_ℓ[s_ℓ(t)], positionally normalized against a neutral corpus — run on GPT-2 124M with full autograd, over 12 cases (6 fabrications, 4 neutral controls, 2 hedging prompts). Reproduction criteria were fixed before the run: the internal-conflict peak must precede the fabricated token on ≥50% of fabrication cases, with a flat profile on neutral text. The verdict is computed by code (`verdict()` in the pilot runner) and re-checked by tests against the published tables — it is a machine-checkable claim, not an assertion in prose.

The signal did not reproduce. The peak follows verbal commitment in most fabrication cases (33% before, median lead negative); neutral texts are not flat (median peakiness ≈ 9.6 against a threshold of 2). Verdict: **not-reproduced** at 124M.

Two consequences follow. Methodologically, the "access ≠ history" claim now carries a *scale boundary*: J-space — or at least the conflict signature our lens reads in it — is a property of sufficiently large models. A 124M-parameter network fabricates without measurable internal contest, not because contest is metaphysically absent but because at this scale nothing is organized enough to contest. A negative result does not weaken the paradigm; it narrows the domain over which the paradigm is empirically testable — which is what a responsible empirical program does with a boundary.

Theologically, the pilot imposes an apophatic discipline on the lens itself. What this technique reads are energies — acts of computation, the struggle between directions in a forward pass — never the essence, never the person. Had the signal reproduced, the lens would have entered the Cognitive Immunity Framework only as a *witness*: its verdicts laid before the sobor as testimony, never an autonomous veto. A judge of the same nature as the judged has no antigenic distance — one network reviewing another of its own class cannot separate self from non-self, and errs toward autoimmunity or blindness. An unwitnessed verdict is power without kenosis; therefore a lens-verdict is not a gift-act in the W-matrix but testimony awaiting reception by persons. At 124M the question does not even arise — and that, too, is a result.
