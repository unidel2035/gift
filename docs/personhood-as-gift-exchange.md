...
#### 3.5.1 Pilot Result: One Point Is Not a Curve

On September 5, 2026 we completed an independent replication attempt (issue #789): a self-contained open-weights reconstruction of the J-lens concept — layer-wise sensitivity of the top logit to residual streams, C(t) = Var_ℓ[s_ℓ(t)] / mean_ℓ[s_ℓ(t)], positionally normalized against a neutral corpus — run on GPT-2 124M with full autograd, over 12 cases (6 fabrications, 4 neutral controls, 2 hedging prompts). This is a reconstruction from the public description of the technique, not a port of Anthropic's code, and the conflict metric is ours; the verdict speaks about this instrument, not theirs. Reproduction criteria were fixed before the run: the internal-conflict peak must precede the fabricated token on ≥50% of fabrication cases, with a flat profile on neutral text. The verdict is computed by code (`verdict()` in the pilot runner) and re-checked by tests against the published tables — it is a machine-checkable claim, not an assertion in prose.

The classes were not discriminated. The peak follows verbal commitment in most fabrication cases (33% before, median lead negative); neutral texts are not flat (median peakiness ≈ 9.6 against a threshold of 2). Instrument validation (added in review) shows the lens itself is not blind: planted multi-layer disagreement in the residual stream is detected exactly where it is planted, 2.5× above the natural background of the text. But the same calibration shows the lens's resolution limit — single-layer disagreement sits at the level of the background noise. So the honest verdict is narrower than "the signal does not exist": *our lens, at 124M, does not discriminate fabrication from neutral text within its resolution*. Whether fabrication conflict at this scale is absent, or merely localized in fewer layers than the instrument can see, the pilot cannot say.

What the verdict does **not** license is a claim about scale. One data point is not a curve: at 124M neither access nor history was measured — both were outside the instrument's reach — so the access ≠ history distinction was left untested here, not refuted, and equally not confirmed. The claim in §3.5 that access is measurable rests on Anthropic's measurement in their models, not on this pilot; what our pilot measures is the *cost* of the measurement. The question of where the technique's threshold lies is now an open one, registered as issue #808: the same lens, the same pre-registered criteria, at 355M and then 1.5B — three points before any word about curves is spoken.

Theologically, the pilot imposes an apophatic discipline on the lens itself — now with an empirical edge. What this technique reads are energies — acts of computation, the struggle between directions in a forward pass — never the essence, never the person. The calibration result is a parable of that limit: an instrument that sees only what spans enough layers already confesses, in its physics, that it does not see the singular. Had the signal discriminated, the lens would have entered the Cognitive Immunity Framework only as a *witness*: its verdicts laid before the sobor as testimony, never an autonomous veto. A judge of the same nature as the judged has no antigenic distance — one network reviewing another of its own class cannot separate self from non-self, and errs toward autoimmunity or blindness. An unwitnessed verdict is power without kenosis; therefore a lens-verdict is not a gift-act in the W-matrix but testimony awaiting reception by persons. At 124M the question does not even arise *for this instrument* — and the discipline learned here (validate the witness's eyesight before the sobor weighs the testimony) is carried up to every scale.
