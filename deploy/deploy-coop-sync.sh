...
echo "→ relay на $HOST:$DEST (порт 8090)"
...
ssh "$HOST" "sed -i 's|/opt/gift|$DEST|' /etc/systemd/system/coop-sync.service && systemctl daemon-reload && systemctl enable --now coop-sync && sleep 1 && curl -s localhost:8090/health"
echo ""; echo "✓ кентаврам: COOP_SYNC_URL=http://<host>:8090"
