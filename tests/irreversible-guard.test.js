 ...
 *
 * Контраст с Emma (Mo Gawdat, emma.love): рекомендательный движок правит
 * прошлое задним числом — пересчитывает совместимость под новый матч.
...
test('irreversible-guard: матрица W доказуемо append-only', async (t) => {
  ...
  await t.test('declined-акт заморожен — мутация веса бросает', () => {
    ...
    // Emma переписала бы вес под новый матч. Здесь — TypeError.
  ...
  // от Emma, правящей прошлое. Когда #62 приземлится — этот тест сторожит
