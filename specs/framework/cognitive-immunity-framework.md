...
> **Кандидат 1.4, не интегрирован: J-lens (слой весов).** Пилот #789
> (05.09.2026, GPT-2 124M, собственная реализация на torch autograd):
> сигнал **не воспроизвёлся** — пик C(t) после коммитмента на большинстве
> фабрикаций (33% < 50%), нейтральные не плоские (медиана peakiness ~9.6
> при пороге 2). Вердикт вычислен кодом и закреплён тестами —
> `experiments/j-lens-pilot/RESULTS.md`. Поведенческая эвристика
> (Antibodies → CrossCheck → LLM-Detector) остаётся единственным
> работающим слоем распознавания.
>
> Границы будущего слоя (если сигнал воспроизведётся на большем масштабе):
> вердикты линзы — **свидетельства для собора** (Layer 3, Мф 18:15), не
> акты в W и не автономное вето: NN, судящая NN своего класса, не имеет
> антигенного расстояния (аутоиммунитет либо слепота), асимптота
> гетерогенности — формальный детерминированный слой (formal-ai). Лицо,
> отвечающее за вето, инкарнируется в W; безликая линза власти не имеет.
