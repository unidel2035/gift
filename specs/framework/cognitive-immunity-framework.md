...
> (05.09.2026, GPT-2 124M, собственная **реконструкция** концепта
> на torch autograd — не порт anthropics/jacobian-lens, метрика своя):
> классы **не дискриминируются** линзой на этом масштабе — пик C(t)
> после коммитмента на большинстве фабрикаций (33% < 50%), нейтральные
> не плоские (медиана peakiness ~9.6 при пороге 2). Прибор при этом
> валиден: positive control детектирует посаженное многослойное
> разногласие ровно в точке посадки (2.5× фона); однослойное — ниже
> разрешения прибора. Значит вердикт — о данных 124M в пределах
> разрешения линзы, не «сигнала нет вообще»: одна точка масштаба
> не говорит о форме кривой — граница масштаба открыта, #808
> (355M → 1.5B). Вердикт вычислен кодом и закреплён тестами —
> `experiments/j-lens-pilot/RESULTS.md` (пин окружения внутри).
> Поведенческая эвристика (Antibodies → CrossCheck → LLM-Detector)
> остаётся единственным работающим слоем распознавания.
>
...
> Дисциплина ревью: зрение свидетеля проверяется ДО взвешивания
> показаний — positive control обязателен на каждом масштабе.
